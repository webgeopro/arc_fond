...
class UserModule extends CWebModule
{
	 ...
	 */
	public function getCabAddress()
	{
        if (Yii::app()->user->isGuest) {
            $address = '';
        } else {
            $tempUser = User::model()->notsafe()->findByPk(Yii::app()->user->id);
            if ($tempUser->profile->form) {  // Юр. лицо form > 0
                $address = 'http://'. $tempUser->username . '.' . $_SERVER["SERVER_NAME"];
            } else { // Частное лицо form=0
                $address = '/id'.Yii::app()->user->id;
            }
        }
        return $address;
    }
